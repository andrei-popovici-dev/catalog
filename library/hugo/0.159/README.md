...
```bash
kraft run -M 512M -p 1313:1313 --plat qemu --arch x86_64 unikraft.org/hugo:0.159
```
...
```bash
kraft run --rm -M 512M -p 1313:1313 --plat qemu --arch x86_64 unikraft.org/hugo:0.159
```
...
In order to to build a local image, clone this repository and `cd` into this directory.
